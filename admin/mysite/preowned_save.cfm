...
<!--- $Id: preowned_save.cfm,v 1.13 2000/06/22 15:50:46 jkrauss Exp $ --->
<!--- preowned queries to update dealerwebs info & insert or update Hours of Operation --->
...
<cfquery name="UpdateDealer" datasource="#gDSN#">
	UPDATE DealerWebs
	SET 
		<cfif #form.preownedsetaparttext# eq "Please enter your text here.">
			preownedSetApartText = ''
		<cfelse>
			preownedSetApartText = '#replace(form.preownedsetaparttext, chr(13), "<BR>", "ALL")#'
		</cfif>
	WHERE DealerCode='#g_dealercode#'
</cfquery>

<!--- in preowned_main, there are 2 Save buttons.  
...
<cfif parameterexists(form.saveall.x)>
	...
	<cfloop index="countDays" from="1" to="7">
		<cfquery name="updHour#countDays#" datasource="#gDSN#">
			...
			SET		UsedOpen = '#HoursArray[countDays][1]#',
					UsedClose = '#HoursArray[countDays][2]#'
			WHERE 	HoursOfOperation.DealerCode = '#g_dealercode#'
			AND 	DayOfWeek = CONVERT(int,#countDays#)
		</cfquery>
	</cfloop>

<!--- 	<cfif form.kbb eq "Y" and form.kbbCode EQ "">
		<cflocation url="preowned.cfm?needkbbcode=yes" addtoken="no">
	<cfelseif form.kbb eq "N" and form.kbbCode EQ "">
		<cflocation url="preowned.cfm" addtoken="no">
	<cfelse>
		...
		<cfif getKBB.RecordCount>
			<cfquery name="updateKBB" datasource="#gDSN#">
				UPDATE 	DlrXRef
				SET		kbbCode = '#form.kbbCode#',
						kbbActive = '#form.kbb#'
				WHERE 	DealerCode = '#g_dealercode#'
			</cfquery>
		<cfelse>
			<cfquery name="updateReyRey" datasource="#gDSN#">
				INSERT INTO DlrXRef(
					dealerCode,
					kbbCode)
					kbbActive)
	...
	<cfif #left(accesslevel,1)# EQ application.sysadmin_access>
		...
		<cfif form.autoupload eq "Y" and form.reynoldscode EQ "">
			<cflocation url="preowned.cfm?needreycode=yes" addtoken="no">
		<cfelseif form.autoupload eq "N" and form.reynoldscode EQ "">
			<cflocation url="preowned.cfm" addtoken="no">
		<cfelse>
			...
			<cfif getReyRey.RecordCount>
				<cfquery name="updateReyRey" datasource="#gDSN#">
					UPDATE 	DlrXRef
					SET		reynoldsCode = '#Form.reynoldsCode#',
							reynoldsActive = '#Form.autoUpload#'
					WHERE 	DealerCode = '#g_dealercode#'
				</cfquery>
			<cfelse>
				<cfquery name="updateReyRey" datasource="#gDSN#">
